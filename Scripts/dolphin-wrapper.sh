...
echo "Make directory /var/data/ProjectPlus/Wii if it doesn't exist"
mkdir -p /var/data/ProjectPlus/Wii

echo "Make directory /var/config/ProjectPlus if it doesn't exist"
mkdir -p /var/config/ProjectPlus
...
UserSDCardCreationDate=`stat --format="%W" /var/data/ProjectPlus/Wii/sd.raw`
...
then
    ...
    cp /app/share/dolphin-emu/sys/Wii/sd.raw /var/data/ProjectPlus/Wii/sd.raw
else
    echo "SD card is already at latest version"
fi

echo "Copy /app/share/dolphin-emu/sys/Wii/Launcher to /var/data/ProjectPlus/Wii if it doesn't already exist"
cp -nr /app/share/dolphin-emu/sys/Wii/Launcher /var/data/ProjectPlus/Wii

echo "Copy user config /app/share/dolphin-emu/user/Config/Dolphin.ini and GFX.ini to /var/config/ProjectPlus/Dolphin.ini and GFX.ini if they don't already exist"
cp -n /app/share/dolphin-emu/user/Config/Dolphin.ini /var/config/ProjectPlus/Dolphin.ini
cp -n /app/share/dolphin-emu/user/Config/GFX.ini /var/config/ProjectPlus/GFX.ini
...
UserHDTexturesCreationDate=`stat --format="%W" /var/data/ProjectPlus/Load/Textures/RSBE01`
...
then
    # Make /var/data/ProjectPlus/Load/Textures directory if it doesn't exist
    echo "Make directory /var/data/ProjectPlus/Load/Textures if it doesn't exist"
    mkdir -p /var/data/ProjectPlus/Load/Textures

    # Copy newer HD textures to user data directory
    echo "Copying newer HD textures to user data directory"
    cp -r /app/share/dolphin-emu/user/Load/Textures/RSBE01 /var/data/ProjectPlus/Load/Textures
else
